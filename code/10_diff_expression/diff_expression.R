...
res_df$diffexp_sig <- factor(res_df$diffexp_sig, levels = diffexp_levels)
table(res_df$diffexp_sig)
...
res_df$sig <- ifelse(res_df$padj < 0.1 & abs(res_df$log2FoldChange) > 1, "yes", "no")
...
top10_up <- res_df[res_df$padj < 0.1 & res_df$log2FoldChange > 1, ]
top10_up <- top10_up[order(top10_up$padj), ][1:10, ]

# Select top 10 downregulated genes
top10_down <- res_df[res_df$padj < 0.1 & res_df$log2FoldChange < -1, ]
top10_down <- top10_down[order(top10_down$padj), ][1:10, ]

# Combine
top10_labeled <- rbind(top10_up, top10_down)
top10_labeled$gene <- rownames(top10_labeled)

# Plot with labels
ggplot(res_df, aes(x = log2FoldChange, y = -log10(padj), color = volcano_color)) +
  geom_point(alpha = 0.6) +
  scale_color_manual(
    values = c("up" = "red", "down" = "blue", "ns" = "gray70"),
    labels = c(
      "up" = "Upregulated (FDR < 0.1)",
      "down" = "Downregulated (FDR < 0.1)",
      "ns" = "Not significant"
    ),
    name = "Significance"
  ) +
  ...
  labs(
    title = "Volcano plot with top 10 up- and downregulated genes",
    x = "log2FoldChange",
...
gff <- read.delim("n_japonicum.emapper.decorated.gff", header = FALSE, sep = "\t", stringsAsFactors = FALSE, comment.char = "#")
colnames(gff) <- c("seqid", "source", "type", "start", "end", "score", "strand", "phase", "attributes")

# Extract NJAPM -> NJAPG mapping from attributes
gene_mrna <- gff[gff$type == "mRNA", "attributes"]
gene_mrna_df <- data.frame(
  mRNA_ID = str_match(gene_mrna, "ID=([^;]+)")[, 2],
  gene_ID = str_match(gene_mrna, "Parent=([^;]+)")[, 2],
  stringsAsFactors = FALSE
)

# Load annotation file (eggNOG-mapper output) and join with transcript -> gene mapping
annotations <- read.delim("n_japonicum.emapper.annotations", header = TRUE, stringsAsFactors = FALSE)
annotations_mapped <- merge(gene_mrna_df, annotations, by.x = "mRNA_ID", by.y = "query")
...
annotations_per_gene <- annotations_mapped %>%
  ...
  ungroup()
colnames(annotations_per_gene)[1] <- "GeneID"

# Merge annotation into top DE gene tables
top10_up$GeneID <- rownames(top10_up)
top10_down$GeneID <- rownames(top10_down)

top10_up_annot <- merge(top10_up, annotations_per_gene, by = "GeneID", all.x = TRUE)
top10_down_annot <- merge(top10_down, annotations_per_gene, by = "GeneID", all.x = TRUE)

# Final format for report tables
final_cols <- c("GeneID", "Gene_Name", "baseMean", "log2FoldChange", "padj", "Description", "COG_category")
top10_up_final <- top10_up_annot[, final_cols]
top10_down_final <- top10_down_annot[, final_cols]

write.table(top10_up_final, "top10_upregulated.txt", sep = "\t", quote = FALSE, row.names = FALSE)
write.table(top10_down_final, "top10_downregulated.txt", sep = "\t", quote = FALSE, row.names = FALSE)
